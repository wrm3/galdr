...
- **SETUP** — first-time wizard. Creates `.galdr/release_profiles/{tier}.yaml` files, scaffolds `template_{tier}/` directories, writes tier metadata into `.galdr/.identity`.
...
1. Reads `.galdr/release_profiles/*.yaml` to learn the tier list
2. Scans each `.galdr/subsystems/*.md` spec
...
Ships in `template_full/` and `template_adv/` only — NOT in `template_slim/`. Slim projects are pre-configured by galdr itself; they do not manage their own tier definitions.
