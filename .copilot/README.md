# .copilot/ — galdr Copilot Platform Config

## Phase 1: Fully Compatible Target (Current)

GitHub Copilot is supported as galdr's **6th IDE platform** at the "compatible" level.
...
| Agents | `.github/agents/` | ✅ Active | 10 galdr agents |
| Hooks | `.github/hooks/galdr-hooks.json` | ✅ Active | JSON format |
...
`.copilot/commands/` is a **galdr convention** — it stores command reference docs for human and agent lookup. GitHub Copilot itself does not read this directory.
...
3. `.github/hooks/galdr-hooks.json` — lifecycle hook JSON wrapper
4. `.github/prompts/` — from `.cursor/commands/*.md` (renamed to `.prompt.md`)

### Phase 2 (Future)

Phase 2 will wire galdr MCP tools (`galdr_valhalla` public URL) directly to Copilot CLI, enabling full slash command support and deeper agent capabilities. See task149 for the implementation plan.
