...
name: g-skl-oracle
description: >
  Execute Oracle Database queries and operations via galdr_valhalla MCP tools.
...
triggers:
  ...
  - g-oracle
requires:
  - galdr_valhalla MCP server (Docker backend)
...
**Requires**: galdr_valhalla Docker backend + `oracle_query` / `oracle_execute` MCP tools.
...
Oracle credentials are **never stored in .galdr/ or vault files**. Always:
...
galdr_server_status()
```
If unavailable: **this skill does not work in slim or full tier installs** — Oracle access requires the galdr_valhalla Docker backend.
