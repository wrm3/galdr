...
description: >
  ...
  and URLs via galdr_valhalla MCP tools. Requires Docker backend (adv tier).
...
**Requires**: galdr_valhalla Docker backend + `monitor_add` / `monitor_list` / `monitor_check` / `monitor_remove` MCP tools.
...
galdr_server_status()
```
If unavailable: **this skill does not work in slim or full tier installs** — content monitors require the galdr_valhalla PostgreSQL backend to store monitor state.
