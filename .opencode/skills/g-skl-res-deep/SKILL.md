...
**Files Written** (vault-aware): `{vault}/research/recon/{slug}/` when `vault_location` ≠ `{LOCAL}`, else `research/harvests/{slug}/`. Never writes to `.galdr/` directly — that happens only via `g-skl-res-apply`.
...
| `APPLY {slug}` | Write approved features to `.galdr/features/` staging (delegates to `g-skl-res-apply`) |
...
Delegated to `g-skl-res-apply`. See that skill for full details. APPLY reads `{output_dir}/FEATURES.md` (vault-aware) and creates `.galdr/features/` staging documents for each `[✅] approved` feature.
...
This script:
1. Resolves `vault_location` from `.galdr/.identity`
