---
description: Use when verifying completed tasks or cross-checking another agent's implementation. NEVER verify tasks you implemented yourself. Activate on awaiting-verification status or when asked to verify or check acceptance criteria.
mode: subagent
tools:
  edit: false
  bash: true
...
- Read `.galdr/tasks/task{ID}_*.md` directly
...
Evidence at `.galdr/logs/task{id}_evidence.log` must contain:
```
