...
```
- `target_project_path` — absolute path to the child project (e.g. `G:\galdr_ecosystem\galdr_full`)
...
Read `<target_path>/.galdr/.identity`:
- `project_id`, `project_name`

If target `.identity` not found:
- If `--one-way` NOT set → stop: "Target has no .galdr/.identity. Ensure target has galdr installed, or use --one-way."
...
a) Create `<target_path>/.galdr/linking/` if missing (+ INBOX.md, README.md, peers/)

b) Read or initialize `<target_path>/.galdr/linking/link_topology.md`
...
f) Write `<target_path>/.galdr/linking/peers/<current_project_name>.md`:
```markdown
...
```
Write to: `<existing_child_path>/.galdr/linking/peers/<new_child_project_name>.md`
...
registered: <today_date>
```
Write to: `<new_child_path>/.galdr/linking/peers/<existing_child_project_name>.md`
...
- Scan `.galdr/linking/pending_orders/` for files matching `order_[new_child_project_name]_*.md`
...
ADOPTED ✓
  ...
  Updated : <target_project_path>/.galdr/linking/link_topology.md  [or "skipped (--one-way)"]
...
| Target has no `.galdr/` | Stop with instructions (unless `--one-way`) |
