...
### 1. Load Topology
Read `.galdr/linking/link_topology.md` to get parent path.
...
Append a `[REQUEST]` entry to `{parent}/.galdr/linking/INBOX.md`:
```markdown
...
**Request a resource:**
```
@g-pcac-ask galdr_mcp needs to expose the oracle connection pool API for our task 015.
```
