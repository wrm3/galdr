# galdr-phase-sync-check Command
...
2. **Scans .galdr/phases/** to find all phase files
...
- **Phase 1: Foundation** - Header exists but no file
  - Action: Create `.galdr/phases/phase1_foundation.md`

### Status Mismatches
- **Phase 2**: TASKS.md shows [✅] but file has `status: in_progress`
  - Action: Update file to `status: completed`

### Orphan Files (in .galdr/phases/ but no TASKS.md header)
...
3. List all `phase*.md` files in `.galdr/phases/`
