...
**Examples**:
```
@g-pcac-send-to --sibling galdr_valhalla --what .galdr/features/galdr_backend --type features
@g-pcac-send-to --child galdr_payments --what src/payments/ --type code --message "Payment subsystem ready for extraction"
@g-pcac-send-to --parent galdr_full --what .galdr/features/feat-012_auth.md --type features --message "This feature should live in the template"
@g-pcac-send-to --sibling galdr_vault --what research/articles/topic.md --type docs
```
...
|----------|--------------------------|
| `features` | `.galdr/features/` |
| `code` | mirror source structure (e.g. `src/foo/` → `<dest>/src/foo/`) |
| `ideas` | `.galdr/IDEA_BOARD.md` (append) |
| `bugs` | `.galdr/BUGS.md` (append summary) + `.galdr/bugs/` (copy files) |
| `docs` | `docs/` |
| `spec` | `.galdr/specifications_collection/` |
...
```
□ Current project has .galdr/.identity
...
```
Check .galdr/linking/link_topology.md for <project_name> in:
  parent | children[] | siblings[]

If found: use recorded project_path
If NOT found: attempt to resolve as <ecosystem_root>/<project_name>
  If that path exists and has .galdr/.identity → treat as accessible, warn "not yet in topology"
...
**For type = ideas**:
- Parse `.galdr/IDEA_BOARD.md` entries that match `--what` (if a file path is given)
  OR read the file if `--what` is a raw ideas .md file
- Append each idea as a new entry to `<dest_project>/.galdr/IDEA_BOARD.md`:
  ```markdown
...
**For type = bugs**:
- Copy bug files to `<dest_project>/.galdr/bugs/`
- Append summary lines to `<dest_project>/.galdr/BUGS.md`

### Step 4 — Update FEATURES.md in destination (if type = features)

Scan the newly copied feature files in `<dest_project>/.galdr/features/`:
- Parse YAML frontmatter (id, title, status, goal)
- Append entries to `<dest_project>/.galdr/FEATURES.md` index table
- Update `Next feat ID` in FEATURES.md if needed

### Step 5 — Write INBOX notification in destination

Append to `<dest_project>/.galdr/linking/INBOX.md` (create file if missing):
...
If `<dest_project>/.galdr/linking/INBOX.md` doesn't exist, create it with a standard header first.

### Step 6 — Log provenance in source project

Append to `<current_project>/.galdr/vault/log.md` (create if missing):
...
"later" = add a reminder task to .galdr/tasks/
```
...
✅ SEND COMPLETE
  ...
  To      : <dest_project>/<dest_path>
  INBOX   : entry written at <dest_project>/.galdr/linking/INBOX.md
  Source  : kept | deleted
...
| Destination has no .galdr/ | Warn: "Destination has no galdr setup. Files will be copied but no INBOX entry written." |
