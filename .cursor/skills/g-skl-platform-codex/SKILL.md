...
description: Authoritative reference for OpenAI Codex CLI customization in galdr projects. Covers .codex/ folder layout, approval modes, commands, MCP, and install verification.
...
Activate for: setting up Codex CLI in a galdr project, authoring `.codex/` configs, understanding approval modes, verifying Codex parity, or answering questions about Codex CLI's capabilities.
...
**galdr target**: Minimal primitive support — commands and skills only. No native rules directory. No hooks. Config-heavy platform.
...
2. **Skills come from `.claude/skills/`** — Codex auto-discovers `.claude/skills/`. The `.codex/skills/` folder is kept for galdr parity tracking but Codex reads `.claude/skills/` directly.
3. **Approval mode for galdr hooks** — galdr's PS1 hooks require shell execution. In `suggest` mode, Codex won't run them. Use `auto-edit` or `full-auto` for full galdr integration.
...
✅ .codex/skills/ has galdr core skills
...
✅ Approval mode: codex --approval-mode auto-edit (for galdr file operations)
```
