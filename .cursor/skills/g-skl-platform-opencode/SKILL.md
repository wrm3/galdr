...
description: Authoritative reference for OpenCode (sst.dev) customization in galdr projects. Covers .opencode/ folder layout, opencode.json config, skills auto-discovery from .claude/skills/, commands, and install verification.
...
Activate for: setting up OpenCode in a galdr project, authoring `.opencode/` configs, understanding opencode.json, verifying OpenCode parity, or answering questions about OpenCode's capabilities.
...
**galdr target**: Full primitive support via `.opencode/` + `opencode.json` conventions. Skills come from `.claude/skills/` automatically.
...
2. **Rules via glob, not copy** — OpenCode reads `.cursor/rules/*.mdc` directly via the `instructions` glob. Do NOT copy rules into `.opencode/rules/` — there is no such directory in the galdr convention.
3. **Skills from `.claude/skills/`** — OpenCode auto-discovers `.claude/skills/`. The `.opencode/skills/` folder does not exist in galdr; skills propagate through `.claude/`.
...
✅ .claude/skills/ has galdr core skills (OpenCode reads from here)
