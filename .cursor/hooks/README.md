...
| Hook | Event | Does |
|------|-------|------|
| `g-hk-session-start.ps1` | Session start | Injects "gald3r is active + check TASKS.md" into every session. Handles first-time user setup. Auto-heals `.gald3r/.project_id`. |
| `g-hk-agent-complete.ps1` | Agent stop | Writes a local chat transcript to `.gald3r/logs/` and a session reminder for next session if 5+ turns occurred. |
...
or the local Cursor chat transcripts in `.gald3r/logs/`. Add `@g-setup` or check
...
gald3r slim now keeps a local copy of Cursor chat history without Docker MCP.

- The `stop` hook calls `.cursor/hooks/g-hk-cursor-chat-logger.py`
- The logger reads Cursor's local `state.vscdb`
- It writes a human-readable transcript to `.gald3r/logs/*_cursor_chat.log`

This is intentionally limited to chat history only. Tool-call auditing and memory
ingestion remain full-version features.
