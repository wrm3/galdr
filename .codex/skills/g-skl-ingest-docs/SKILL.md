...
All notes written by this skill must conform to **`.galdr/vault/VAULT_OBSIDIAN_STANDARD.md`** (or the copy at `{vault_location}`): §1 frontmatter, §3 tags (`platform-doc` + platform slug + path-derived topics), UTF-8 **no BOM**.
