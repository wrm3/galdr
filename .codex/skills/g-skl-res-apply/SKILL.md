...
version: 1.1.0
description: >
  Convert a reverse-spec FEATURES.md (produced by g-skl-res-deep) into galdr
...
`g-skl-res-deep` writes nothing to `.galdr/` — it produces structured `FEATURES.md` reports for human review.

`g-skl-res-apply` is the **execution layer**: it reads a `FEATURES.md`, groups features into coherent galdr artifacts, and writes them to the project. It is called after the human has reviewed and approved features (changing their status to `[✅]`).
...
Like `g-skl-res-deep` and `g-skl-res-review`, this skill resolves the recon base path from `.galdr/.identity` before reading input.

### Path resolution (per C-003)

```
1. Read .galdr/.identity (key=value, no quotes)
...
4. **If <30% overlap** → create NEW subsystem spec at `.galdr/subsystems/{slug}.md`
...
```
g-res-apply APPLY {slug} --target galdr_valhalla
g-res-apply APPLY {slug} --target galdr_valhalla --mode=approved
```

**Behavior when `--target` is set:**

1. **Resolve target project path** from `.galdr/linking/link_topology.md`:
...
2. **Check target's CONSTRAINTS.md** — read `{target_path}/.galdr/CONSTRAINTS.md`:
...
3. **Write artifacts to target project** — same as standard APPLY but with all paths prefixed with `{target_path}/`:
   - `{target_path}/.galdr/PROJECT.md`
   - `{target_path}/.galdr/features/`
   - `{target_path}/.galdr/FEATURES.md`
   - `{target_path}/.galdr/TASKS.md`
   - `{target_path}/.galdr/tasks/`
   - `{target_path}/.galdr/SUBSYSTEMS.md`
   - `{target_path}/.galdr/subsystems/`

4. **Send PCAC INFO notification** to target project INBOX (`{target_path}/.galdr/linking/INBOX.md`):
   ```
   [INFO] g-res-apply APPLY from galdr_full: {N} features applied from {slug} harvest.
   Review: {target_path}/.galdr/tasks/ for new task files.
...
5. Summary printed at end:
   ...
   → galdr_valhalla: 3 features (feat-040, feat-085, ...)
   → galdr_agent: 2 features (feat-029, ...)
...
1. Queue the APPLY as a **pending PCAC order** in `.galdr/linking/pending_orders/`
...
```bash
# Apply all approved features to galdr_valhalla (backend project)
g-res-apply APPLY hermes-agent__nousresearch --target galdr_valhalla --mode=approved
...
g-res-apply APPLY hermes-agent__nousresearch --target galdr_valhalla --force
```
