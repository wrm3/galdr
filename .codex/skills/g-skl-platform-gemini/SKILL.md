...
description: Authoritative reference for Gemini CLI (Google DeepMind) customization in galdr projects. Covers .agent/ folder layout, GEMINI.md, rules, skills, commands, and install verification.
...
Activate for: setting up Gemini CLI in a galdr project, authoring `.agent/` configs, verifying Gemini parity, or answering questions about Gemini CLI's capabilities.
...
**galdr target**: Full parity with Cursor. Uses `.agent/` folder (also called `.agents/` in some docs). galdr uses `.agent/` as the canonical name.
...
```
.agent/                       ← Gemini CLI config (galdr canonical name)
...
**Note**: Gemini reads both `.agent/` and `.agents/` — galdr uses `.agent/` for consistency.
...
1. **`.agent/` vs `.agents/`** — galdr uses `.agent/` but Gemini reads both. Do not create both; `.agent/` is canonical.
...
4. **Memory via GEMINI.md** — Gemini's "memory" feature appends to `GEMINI.md`. Be careful not to let galdr rules be overwritten by Gemini's memory injections.
...
✅ .agent/skills/ has galdr core skills  
