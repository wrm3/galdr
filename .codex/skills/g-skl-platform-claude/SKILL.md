...
description: Authoritative reference for Claude Code customization in galdr projects. Covers .claude/ folder layout, CLAUDE.md, agents, skills, commands, hooks (hooks.json format), MCP, and install verification.
...
Activate for: setting up Claude Code in a galdr project, authoring `.claude/` configs, understanding hooks.json format, verifying Claude parity, or answering questions about Claude Code's capabilities.
...
**galdr target**: Full parity with Cursor. Uses `.md` extension for rules (not `.mdc`).
...
**Key**: `.claude/skills/` is auto-discovered by Claude Code, OpenCode, AND GitHub Copilot — making it the most broadly supported skill location in the galdr ecosystem.
...
✅ .claude/skills/ has galdr core skills
