...
| Scope | File | Use for |
|-------|------|---------|
| Project | `.galdr/learned-facts.md` | Project-specific facts, decisions, gotchas |
...
1. Read `.galdr/.identity` to get `project_name` and `vault_location`
2. Read `.galdr/learned-facts.md`
...
1. Show all project facts from `.galdr/learned-facts.md`
...
Required: `type: decision` and category tag `decision` in `tags:` per **VAULT_OBSIDIAN_STANDARD.md**. Plain `.galdr/learned-facts.md` bullets stay file-only (no YAML).
