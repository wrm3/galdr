...
```
# Adopt galdr_full as a child project
@g-pcac-adopt G:\galdr_ecosystem\galdr_full

# Adopt without touching the other project (remote/read-only)
@g-pcac-adopt G:\galdr_ecosystem\galdr_mcp --one-way
```

## What Happens

1. Reads `.galdr/.identity` from both this project and the target
2. Creates `.galdr/linking/` in this project if missing
